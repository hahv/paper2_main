...
\begin{tikzpicture}[
    ...
    \draw[->, line width=1.5pt] (eager) edge[bend left=25]
        node[below, align=center]{%
            ...
            consecutive negatives \\[2pt]
            \textit{reset:}\;
            $c_{\mathrm{fire}} \leftarrow 0,\;
             c_{\mathrm{skip}} \leftarrow 0$}
        (normal);

    %% ── T2: NORMAL → EAGER via motion streak ─────────────────────────
    \draw[->, line width=1.5pt, red!70!black] (normal) edge[bend left=25]
        node[above, align=center]{%
            \footnotesize
            \textbf{[Motion path]}\;
            $c_{\mathrm{fire}} \geq W_{\mathrm{fire}}$
            consecutive positives \\[2pt]
            \textit{reset:}\;
    ...
    \draw[->, red!70!black] (normal) edge[loop left, looseness=5]
        node[left, align=right]{%
            \footnotesize
            \textbf{Motion:}\; $s_t{=}1$ \\[2pt]
            $c_{\mathrm{skip}} \leftarrow 0$ \\[1pt]
            $\hat{y}_t{=}1$:\;
    ...
    \draw[->, red!70!black] (normal) edge[loop below, looseness=5]
        node[below, align=center]{%
            ...
            \textbf{Forced burst:}\;
            $s_t{=}0$ \textbf{and}
            $c_{\mathrm{skip}} \geq N_{\mathrm{chk}}$ \\[2pt]
            Run $\mathcal{M}$ on next $W_{\mathrm{fire}}$
            consecutive frames \\[2pt]
            \textit{all positive}:\;
            $\to$ \textbf{EAGER}
            \quad
            \textit{any negative}:\;
    ...
    %% ── EAGER top loop: positive (stay alert) ───────────────────────
    ...
    %% ── EAGER bottom loop: negative (counting clears) ───────────────
    \draw[->] (eager) edge[loop below, looseness=5]
        node[below, align=center]{%
            \footnotesize
    ...
    \node[draw, rounded corners, fill=gray!8,
          below=2.8cm of normal,
          anchor=north west, xshift=-4.5cm,
        ...
        \begin{tabular}{@{}lp{11cm}@{}}
            ...
            $c_{\mathrm{fire}}$
                & Consecutive positive $\mathcal{M}$ results on motion
                  path; reset on any negative or T1 exit \\[3pt]
            $c_{\mathrm{clear}}$
            ...
            $c_{\mathrm{skip}}$
                & Consecutive skipped frames in \textsc{Normal};
                  reset on any $\mathcal{M}$ call \\[3pt]
            $W_{\mathrm{fire}}$
                & (1)~Motion streak length to enter \textsc{Eager};\;
                  (2)~Burst size for forced check \\[3pt]
            $W_{\mathrm{clr}}$

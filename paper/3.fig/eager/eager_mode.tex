...
\begin{tikzpicture}[
    ...
    \draw[->, red!70!black] (normal) edge[loop below, looseness=5]
        node[below, align=center]{%
            ...
            \textit{if:}\; $s_t{=}0$ \textbf{and}
                $c_{\mathrm{skip}} \geq N_{\mathrm{chk}}$ \\[2pt]
            \textit{then:}\; $c_{\mathrm{skip}} \leftarrow 0$;\;
                run $\mathcal{M}$ on up to $W_{\mathrm{fire}}$ frames \\[1pt]
            \phantom{\textit{then:}\;}
                \textit{(as if $s_t{=}1$; enter \textbf{EAGER} early if $c_{\mathrm{fire}}$ reaches $W_{\mathrm{fire}}$)}}
        (normal);

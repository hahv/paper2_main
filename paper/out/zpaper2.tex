...
Section\textasciitilde{}\ref{sec:method}.
...
the WiseNet dataset \citeproc{ref-marroquin2019wisenet}{{[}30{]}}.
Table\textasciitilde{}\ref{tb:videodb} summarizes the properties of the
self-collected video dataset alongside a comparison with existing
benchmark datasets, and Figure\textasciitilde{}\ref{fig:videodb}
presents representative sample frames.

For the hyperparameter optimization described in
Section\textasciitilde{}\ref{sec:hyperparam}, the video dataset was
further partitioned into a validation set, \(D_{\text{val}}\) (46
videos), used to select the optimal skip-module hyperparameters, and a
test set, \(D_{\text{test}}\) (104 videos), used for the final
evaluation of the skip-enabled system against the baseline and existing
methods. The split followed a 30:70 ratio (validation:test), following
the protocol of \citeproc{ref-de2023hybrid}{{[}10{]}}, and was performed
using stratified sampling to preserve balanced class distributions in
both subsets.
...
\citeproc{ref-dfiredataset}{{[}20{]}} with additional fire and smoke
...
\item
  ...
  \citeproc{ref-jadon2019firenet}{{[}16{]}}:} A MobileNetV2-based
...
\item
  ...
  \citeproc{ref-de2023hybrid}{{[}10{]}}:} A YOLOv8-Nano object detector
  trained on a small dataset (\$\sim\$2k images). It offers localization
  but struggles with small or semi-transparent smoke features due to
  limited training data.
\item
  \textbf{M3 (Temporal Voting Method)
  \citeproc{ref-de2023hybrid}{{[}10{]}}:} A video-level approach that
  aggregates inference results over a sliding window of 30 frames to
  reduce false alarms. While effective for reducing noise, it introduces

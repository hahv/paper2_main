...
$R_{\text{base}}$ and $T_{\text{DL}}$ denote the recall and mean per-frame
inference time of the baseline pipeline (without skipping), respectively. For
...
\rightarrow [\mathcal{M}]$ and compute three metrics: recall $R(\theta)$, skip
rate $S_r(\theta)$, and mean per-frame skip module time $T_{\text{skip}}(\theta)$.
...
the skip module $\mathcal{S}$ assigns $s_t = 0$ --- that is, background frames
correctly identified as having no motion and therefore bypassed without invoking
$\mathcal{M}$ --- and $N_{\text{neg}}$ is the total number of

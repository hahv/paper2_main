...
Table \ref{tb:val_results_accMotionDet} shows the top-10 ranked
configurations for the AccMotionDet skip module on the validation set
\(\mathcal{D}_\text{val}\), ordered by combined score \(\Phi\). The
...
scale (\(\alpha = 0.5\)), coarse block size (\(B = 32\)), conservative
block-active threshold (\(\tau = 0.05\)), moderate pixel sensitivity
(\(\tau_d = 5\)), low activation threshold (\(\tau_m = 5\)), and a small
accumulation cap (\(K_{\max} = 15\)). This configuration achieves a skip
rate of \textbf{44.39\%} and a recall of \textbf{93.90\%}
...
(\(\alpha = 1.0\)) does not appear in any feasible high-scoring
configuration, confirming that downscaling reduces sensitivity to
high-frequency pixel noise while lowering skip-module latency --- both
...
evaluated (\(K_{\max} \in \{15, 25, 35\}\)), the skip rate decreases
monotonically with increasing \(K_{\max}\) (e.g., 44.39\% \(\to\)
42.89\% \(\to\) 42.38\% for Exp.\textasciitilde1--3). A lower cap allows
the accumulator to reset more readily after high-motion periods,
enabling faster recognition of subsequent static frames and thus higher
skip rates. The difference in combined score across the three values is
small (\(\leq 0.007\)), indicating low sensitivity to this parameter
...
\paragraph{Eager Mode: Recovery of Missed
Cases}\label{eager-mode-recovery-of-missed-cases}

\textbf{Fix for slow-onset smoke:} We intro the skip module with eager
mode and periodically check for specific scences
...
We do hyper-parameter search on \(D_\text{val}\) to find the optimal
params for this eager mode.

\begin{table*}[hbtp]
    \caption{Validation-set performance and hyperparameter configurations for
    Accumulation Motion Detector in Eager Mode (AccMotionDet Eager). Note that the
    ``Recall (\%)'' column displays the retained recall alongside the absolute
    recall drop relative to the baseline in parentheses, i.e., Recall (-Recall
    Drop). The best and second-best results for each metric are highlighted in
    \textbf{bold} and \textit{\underline{italic underline}}, respectively.
    ...
    centered tabularray = {
        width = \textwidth,
        colspec = {X[2.5,c]X[c]X[c]X[1.5,c]X[c]X[1.5,c]X[c]X[1.5,c]},
        colsep = {1pt},
        hlines,
        vlines,
        cells = {font = \scriptsize, halign = c, valign = m},
        ...
        cell{1}{2} = {c=2}{c},   % Parameters
        cell{1}{4} = {c=4}{c},   % Metrics
        cell{1}{8} = {r=2}{c},   % Combined Score
    },

...
%% ---------------------------------------------------------------------
%% Packages already provided by the B.tex (elsarticle) template
%% NOTE: algcompatible removed (conflicts with algpseudocode internals)
%% ---------------------------------------------------------------------
\usepackage{graphicx, amsmath, algorithm, algpseudocode, pifont, comment, layout, amsthm, amssymb}
\usepackage{enumitem}
\usepackage{eso-pic}
\usepackage{booktabs}
\usepackage{float}
...
%% A.tex manuscript (equations, algorithms, tables, figures, citations).
%% Only packages NOT already provided above are added here.
%% ---------------------------------------------------------------------
\usepackage{amsfonts}
\usepackage{array}
\usepackage{multirow}
...
%% natbib is loaded automatically by elsarticle. Select numbered,
%% compressed citation style to match the original A.tex ([numbers]
%% option to natbib) and the elsarticle-num bibliography style below.
\biboptions{numbers,sort&compress}

\journal{ICT Express}

%%%Author definitions (acronym helpers carried over from A.tex)
\newcommand{\DL}{\textsc{dl}\xspace}
\newcommand{\CCTV}{\textsc{cctv}\xspace}
\newcommand{\FPS}{\textsc{fps}\xspace}
\newcommand{\FAR}{\textsc{far}\xspace}
...
\author[1]{Van-Ha Hoang\fnref{fn1}}
\ead{hahv@uit.edu.vn}
\fntext[fn1]{Present address: Faculty of Software Engineering, University of Information Technology, Ho Chi Minh city, Vietnam.}

\author[1]{Jong Weon Lee}
\ead{jwlee@sejong.ac.kr}

\author[2]{Goo-Rak Kwon}
\ead{grkwon@chosun.ac.kr}

\author[3]{Chun-Su Park\corref{cor1}}
\ead{cspk@skku.edu}

\address[1]{Department of Software, Sejong University, Seoul, 05006, South Korea}
\address[2]{Department of Information and Communication Engineering, Chosun University, Gwangju, 61452, South Korea}
\address[3]{Department of Computer Education, Sungkyunkwan University, Seoul, 03063, South Korea}
...
\begin{keyword}
Fire detection \sep Smoke detection \sep Skip module \sep Inference
...
estimated economic loss of 10 billion USD \cite{californiafire:online}.
...
\cite{southkoreafire:online}. These incidents highlight the critical
...
\cite{cheng2024visual,gragnaniello2024fire}. In practical deployments,
...
\cite{cheng2024visual} provides a comprehensive survey of visual fire
...
boxes \cite{geng2024yolofm,khan2025optimized}. Research efforts have
...
\cite{khan2025beyond} survey video-based fire and smoke detection
approaches and highlight the benefit of modeling temporal dynamics.
Representative methods include the work of \cite{cao2019attention}, who
combine a CNN for spatial feature extraction with a bidirectional LSTM
for temporal modeling, and \cite{ali2025toward}, who employ 3D CNNs
...
such as temporal voting \cite{de2023hybrid} or motion-based false
positive suppression \cite{gragnaniello2025flame} --- yet still apply
...
\cite{sobral2014comprehensive} and \cite{garcia2020background} provide
...
robust methods such as MOG2 and KNN \cite{zivkovic2006efficient} model
...
\cite{ghodrati2021frameexit}, employ learned gating mechanisms to
...
Inspired by \cite{yu2013real}, we design \textsc{AccMotionDet} to
...
FireNet \cite{jadon2019firenet}, Firesense \cite{Firesens4:online},
FiSmo \cite{cazzolato2017fismo}, and FURG
\cite{steffens2015unconstrained}. However, these were recorded with
moving cameras and are therefore unsuitable for static surveillance
scenarios. Static-camera datasets such as DFire \cite{dfiredataset},
VisiFire Bilkent \cite{VisiFireBilkent:online}, KMU Fire and Smoke
\cite{KMUFireSmokeDataset}, Mivia Fire and Mivia Smoke datasets
\cite{foggia2015real}, and USTC Smoke \cite{lin2017smoke} do exist;
...
and environmental conditions. To address these deficiencies, we
constructed a dedicated static indoor video dataset by aggregating clips
from multiple heterogeneous sources. Fire and smoke samples were sourced
from the Korea AI Fire Dataset \cite{AIHub87:online}, the USTC Smoke
Dataset \cite{lin2017smoke}, and the VDS3 Dataset \cite{huang2022fire},
supplemented by videos collected from open online platforms (Pexels,
Pixabay, and YouTube). Non-fire/smoke (negative) samples were compiled
from self-recorded footage captured by real CCTV cameras in various
indoor environments (e.g., parking areas), along with videos from the
Safe \& Unsafe Behavior in Workplaces dataset \cite{onal2024video}, the
Indoor Action dataset \cite{deniz2024optimized}, the MPII Cooking 2
Dataset \cite{rohrbach2016recognizing}, and the WiseNet dataset
\cite{marroquin2019wisenet}. Table~\ref{tb:ufireindoor} summarizes the
...
\cite{de2023hybrid}, and was performed using stratified sampling to
...
\cite{steffens2016non}. Frame-level evaluation measures detection
...
\begin{itemize}
\item \textbf{MobileNet} \cite{mukhopadhyay2019fpga}: A modified
MobileNet architecture fine-tuned for fire and smoke detection.
\item \textbf{FireNet} \cite{jadon2019firenet}: A lightweight
CNN-based image classifier comprising 14 layers, with a model size of
7.5~MB and approximately 650k trainable parameters.
\item \textbf{YOLOv5s} \cite{de2023hybrid}: A lightweight object
detector based on YOLOv5, trained with hyperparameters selected via
grid search on the D-Fire dataset \cite{dfiredataset}.
\item \textbf{YOLOv5l} \cite{de2023hybrid}: A heavier variant of the
...
\cite{hgnetv2timm:online} from the Timm library \cite{rw2019timm},
which was originally trained using SSLD knowledge distillation
\cite{cui2021beyond}. HGNetV2 \cite{hgnetv2PaddleCl7:online} is a
...
using the Adam optimizer \cite{kingma2014adam}, configured with a
...
neural scaling laws \cite{hestness2017deep,alabdulmohsin2022revisiting,bahri2024explaining}:
...
Temporal Persistence Thresholding (TPT) \cite{de2023hybrid} augments
...
We follow the hyperparameter selection protocol of \cite{de2023hybrid},
...
to 168, the system returns negative predictions yet remains in
...
%\textbf{Van-Ha Hoang:} Writing -- original draft, Data curation,
%Conceptualization, Methodology, Investigation, Formal analysis,
%Software, Visualization.
%\textbf{Jong Weon Lee:} Writing -- review \& editing, Supervision,
%Funding acquisition.
%\textbf{Goo-Rak Kwon:} Writing -- review \& editing, Funding
%acquisition.
%\textbf{Chun-Su Park:} Writing -- review \& editing, Conceptualization,
%Methodology, Formal analysis, Resources, Supervision, Project
...
Republic of Korea, under Grant RS-2025-23323861. This research was
supported by the Technology Development Program (RS-2025-25439892)
funded by the Ministry of SMEs and Startups (MSS), Republic of Korea.
This research was supported by the MSIT, Korea, under the Graduate
School of Virtual Convergence support program
(IITP-2026-RS-2023-00254129), supervised by the IITP (Institute for
Information \& Communications Technology Planning \& Evaluation). This
research was supported by the ANCHOR program through the Seoul ANCHOR
Center, funded by the Ministry of Education (MOE) and the Seoul
Metropolitan Government (2026-ANCHOR-01-018-05). This work was
supported by the IITP--ITRC (Information Technology Research Center)
grant funded by the Korea government (MSIT) (IITP-2026-RS-2022-00156354).
...
%\includegraphics[width=\linewidth]{figs/authors/a1}
%\end{minipage}\hfill
%\begin{minipage}[t]{0.75\columnwidth}
%\textbf{Van-Ha Hoang} received an engineering degree in Software and an
%M.S. degree in Computer Science from the University of Information
...
%\noindent
%\begin{minipage}[t]{0.22\columnwidth}
%\includegraphics[width=\linewidth]{figs/authors/a2}
%\end{minipage}\hfill
%\begin{minipage}[t]{0.75\columnwidth}
%\textbf{Jong Weon Lee} received an M.S. degree in Electrical and
%Computer Engineering from the University of Wisconsin at Madison in
%1991, and a Ph.D. degree from the University of Southern California in
%2002. He is presently a Professor in the Department of Software at
%Sejong University. His research interests include virtual reality,
...
%\noindent
%\begin{minipage}[t]{0.22\columnwidth}
%\includegraphics[width=\linewidth]{figs/authors/a3}
%\end{minipage}\hfill
%\begin{minipage}[t]{0.75\columnwidth}
%\textbf{Goo-Rak Kwon} (Senior Member, IEEE) received the Ph.D. degree
%from the Department of Mechatronic Engineering, Korea University, in
%2007. He was the Chief Executive Officer and the Director of Dali Tech
%Company Ltd., from 2004 to 2007. He joined the Department of Electronic
%Engineering, Korea University, from 2007 to 2008, where he was a
%Postdoctoral Researcher supporting the BK21 Information Technique
%Business. He has been a Professor with Chosun University, since 2017.
%His research interests include medical image analysis, A/V signal
%processing, video communication, and applications. He was a life
%member of KMMS, KICS, and KING.
%\end{minipage}
%
%\bigskip
%\noindent
%\begin{minipage}[t]{0.22\columnwidth}
%\includegraphics[width=\linewidth]{figs/authors/a4}
%\end{minipage}\hfill
%\begin{minipage}[t]{0.75\columnwidth}
%\textbf{Chun-Su Park} received the B.S. and Ph.D. degrees in
%electrical engineering from Korea University, Seoul, in 2003 and 2009,
...
%Sejong University. In 2017, he joined the Department of Computer
%Education, Sungkyunkwan University, where he is currently a Professor.
%His research interests include intelligent vision and AI software.
%\end{minipage}

%% Loading bibliography style and file

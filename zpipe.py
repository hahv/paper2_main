...
# Precompute constants (do this once at module level)
mean_scaled = np.array(
    [0.485 / 255.0, 0.456 / 255.0, 0.406 / 255.0], dtype=np.float32
).reshape(1, 1, 3)
std_scaled = np.array(
...
def extract_all_blocks_cpu_cv2(frame_bgr, block_size=16):
    ...
    frame = frame_bgr[..., [2, 1, 0]]  # (H,W,3)
    zprofiler.step_end("extract_all_blocks_cpu_cv2", "bgr2rgb")

    # 2. Convert to float and normalize
    zprofiler.step_start("extract_all_blocks_cpu_cv2", "normalize")
    frame = frame.astype(np.float32)
    np.multiply(frame, std_scaled, out=frame)  # In-place: frame * (1/(255*std))
    np.subtract(frame, mean_scaled, out=frame)  # In-place: frame - (mean/255)
    zprofiler.step_end("extract_all_blocks_cpu_cv2", "normalize")
    ...
    # 4. Use as_strided to extract blocks
    zprofiler.step_start("extract_all_blocks_cpu_cv2", "unfold")
    C, H, W = tensor.shape
    stride = tensor.strides
    blocks = as_strided(
        tensor,
    ...
    blocks = blocks.reshape(-1, C, block_size, block_size)
    zprofiler.step_end("extract_all_blocks_cpu_cv2", "unfold")

    # 5. Convert to PyTorch tensor and return
    zprofiler.step_start("extract_all_blocks_cpu_cv2", "to_torch")
    result = torch.from_numpy(blocks).float()
    zprofiler.step_end("extract_all_blocks_cpu_cv2", "to_torch")
    return result
...
def skip_module(tiny_model, frame, pil_img_frame):
    ...
    blocks_cpu = extract_all_blocks_cpu_transform(pil_img_frame, block_size=BLOCK_SIZE)
    global device
...
def main():
    ...
    while True:
        ...
        if SKIP_ENABLED:
            should_skip, _, fg_mask = skip_module(tiny_model, frame, pil_img)
    ...
    zprofiler.meta_info()
    zprofiler.report_and_plot(outdir="docs/profiler")

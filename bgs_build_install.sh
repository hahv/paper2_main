source /mnt/e/NextCloud/bgslibrary-examples-python/.venv/bin/python
cd /home/ha/dev/bgslibrary
rm -rf build # Clear previous build to avoid cache issues
mkdir build && cd build
cmake .. \
	-DPYTHON_EXECUTABLE=/mnt/e/NextCloud/bgslibrary-examples-python/.venv/bin/python \
	-DBGS_CORE_STATIC=ON \
	-DBGS_PYTHON_SUPPORT=ON \
	-DBGS_PYTHON_ONLY=ON \
	-DBGS_PYTHON_VERSION=3.11 \
	-DNUMPY_INCLUDE_DIR=$(python -c "import numpy; print(numpy.get_include())") \
	-Dpybind11_DIR=/home/ha/dev/bgslibrary/modules/pybind11 \
	-DCMAKE_INSTALL_PREFIX=/mnt/e/NextCloud/bgslibrary-examples-python/.venv
make -j8
make install


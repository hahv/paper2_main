...
def main():
    ...
    if args.input_img_mode is None:
        input_img_mode = "RGB" if args.input_size[0] == 3 else "L"
    else:
    ...
    args.num_classes = len(dataset_train.reader.class_to_idx)
    ...
    if utils.is_primary(args):
        ...
        output_dir = utils.get_outdir(
            args.output if args.output else "./output", exp_name
        )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # cspk
        latest_metric = None
        if (loader_eval is not None) and (
            (epoch + 1) % bisect_right(val_span, -epoch) == 0
        ):
            ...
            if latest_metric > best_metric:
                ...
                if best_metric > 98 and saver is not None:
                    # if (best_metric > 10 and saver is not None) or (epoch - best_epoch) >= 10:
                    # save proper checkpoint with eval metric
                    saver.save_checkpoint(
                        epoch, metric=latest_metric, best_metric=best_metric
                    )
                    # saver.save_checkpoint(epoch, metric=latest_metric, best_metric=best_metric)
                    last_saved = epoch

                print(
                    "Current: ", latest_metric, epoch, "Best: ", best_metric, best_epoch
                )
            elif epoch - last_saved >= 10:
                saver.save_checkpoint2(
                    epoch, metric=latest_metric, best_metric=best_metric
                )
                print(
                    "Backup: ", latest_metric, epoch, "Best: ", best_metric, best_epoch
                )
                last_saved = epoch
    ...
    _logger.info(f"Dataset: {args.data_dir}")
    _logger.info("*** Best metric: {0} (epoch {1})".format(best_metric, best_epoch))

    train_end_time = time.time()

    consumed_time = str(timedelta(seconds=int(train_end_time - train_start_time)))
    _logger.info(f"End of training. Time consumed: {consumed_time}")
...
group.add_argument(
    ...
    default=5,
    metavar="NORM",
    help="Clip gradient norm (default: 5)",
)
group.add_argument(
    "--clip-mode",
    type=str,
...
group.add_argument(
    ...
    default=5e-4,
    metavar="LR",
    help="learning rate, overrides lr-base if set (default: 5e-4)",
)
group.add_argument(
    "--lr-base",
    type=float,
...
group.add_argument(
    "--hflip", type=float, default=0.0, help="Horizontal flip training aug probability"
)
group.add_argument(

...
class VideoBlockSkipRsProc(VideoInferRsProc):
    ...
    def calc_frame_size(self, video_path: str, **kwargs) -> tuple:
        if self.renderer_context == "original_frame":
            ...
            method_cfg = self.cfg.methodCfg.extra_cfgs.get("skip_proc").get("params")  # ty:ignore[unresolved-attribute]
    ...
    def get_custom_renderer(self) -> List[BaseRenderer]:  # ty:ignore[invalid-return-type]
        method_cfg = self.cfg.methodCfg.extra_cfgs.get("skip_proc")  # ty:ignore[unresolved-attribute]

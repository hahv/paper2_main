...
FireDetector::FireDetector(Config& cfg)
    ...
    if (temporalStabilizationEnable)
        drawBlockDebug = cfg.fdDrawBlockDebug;
    else
        drawBlockDebug = false;
#endif
}

bool FireDetector::runModelSingle(FDRecord& fdRcd, Mat& oriFrame, int vchID, Rect roi, int& detectedClassID) {
    int fireFlag = 0, smokeFlag = 0;
    float* classScores;
    Mat preFrame, roiFrame;

    roiFrame = oriFrame(roi);

    if (roiFrame.size() == Size(netWidth, netHeight)) {
        if(roiFrame.isContinuous())
            preFrame = roiFrame;
        else
            preFrame = roiFrame.clone();
    }
    else
...
#ifndef _CPU_INFER
    if (fdRcd.pts.size() >= 4) {
        vector<Point> rectPts;
        rectPts.push_back(roi.tl());  // top-left
        rectPts.push_back(Point(roi.x + roi.width, roi.y));          // top-right
        rectPts.push_back(roi.br());  // bottom-right
        rectPts.push_back(Point(roi.x, roi.y + roi.height));         // bottom-left

        vector<Point> intersect;
        float intersectArea = intersectConvexConvex(rectPts, fdRcd.pts, intersect);

        if (intersectArea > 0) {
            float scale = std::min((float)netWidth / roiFrame.cols, (float)netHeight / roiFrame.rows);

            vector<Point> scaledPts;
            for (auto& pt : intersect) {
                Point scaledPt;
                scaledPt.x = std::min(std::max(scale * (pt.x - roi.x) + 0.5f, 0.0f), (float)(netWidth - 1));
                scaledPt.y = std::min(std::max(scale * (pt.y - roi.y) + 0.5f, 0.0f), (float)(netHeight - 1));

                scaledPts.push_back(scaledPt);
            }

            cv::fillConvexPoly(preFrame, scaledPts, Scalar(114, 114, 114));
        }
    }
	...
	//printf("%.4f %.4f %.4f\n", classScores[0], classScores[1], classScores[2]);
	...
	float score = probs[classID] / sumExp;
    if (score < reliableTh) //confidence threshold
		classID = FD_CLASS_NONE;
    ...
    detectedClassID = classID; //to return the detected class
...
bool FireDetector::runModel(FDRecord& fdRcd, Mat& frame, int vchID, int& detectedClassID) {
    if (!fdEnable)
        return true;
    ...
    Mat oriFrame;
    Rect roi = Rect(0, 0, frame.cols, frame.rows); // Initialize roi to the full frame    
    ...
    runModelSingle(fdRcd, oriFrame, vchID, roi, detectedClassID);
    return true;
}
...
/* The line `Rect initROI(min_x0, min_y0, max_x1 - min_x0, max_y1 - min_y0);` is creating a `Rect` object named `initROI` with the specified parameters. */

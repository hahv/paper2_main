...
class ProfDSMetricSrc(BaseMetricSrc):
    """
        ...
        if not self.save_raw_metrics:
            ...
            if mode == "per_video":
                ...
                for gt, df in per_video_out_list:
                    ...
                    v_pred = ProfDSMetricSrc.NEG_LABEL
                    preds = df["pred_label"].unique().tolist()
                    if ProfDSMetricSrc.POS_LABEL in preds:
                        v_pred = ProfDSMetricSrc.POS_LABEL
                ...
                dfmk["raw_preds"].sort_values(by=["correct", "video_name"], ascending=[True, True], inplace=True)
                dfmk["raw_preds"].to_csv(
                    os.path.join(
                        self.cfg.get_outdir(), f"[___{mode}]_raw_metric_src_.csv"
                    ),
                    sep=";",
                    encoding="utf-8",
                    index=False,
    ...
    def _load_raw_metric_data(self, **kwargs) -> Dict[str, Any]:
        ...
        for csv_file in csv_files:
            ...
            gt = (
                ProfDSMetricSrc.NEG_LABEL
                if "__lb_none__" in video_name
                else ProfDSMetricSrc.POS_LABEL
            )
            df = pd.read_csv(
                csv_file,
                sep=";",
            ...
            df["pred_label"] = (
                ...
                .apply(
                    lambda x: (
                        ProfDSMetricSrc.POS_LABEL
                        if ("fire" in x or "smoke" in x)
                        else ProfDSMetricSrc.NEG_LABEL
                    )
                )
            )
            # pprint(csv_file)
    ...
    def _get_cls_data(self, metric, mode, **kwargs) -> Dict[str, Any]:
        ...
        if metric == "FPS":
            ...
            for per_video_data in per_video_out_list:
                ...
                preds = np.array(preds) == ProfDSMetricSrc.POS_LABEL
                preds = preds.astype(int).tolist()  # convert to int
                gts = per_video_data[0]  # already numpy
                gts = np.array(gts) == ProfDSMetricSrc.POS_LABEL

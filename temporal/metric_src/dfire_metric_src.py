...
class DFireDSMetricSrc(BaseMetricSrc):
    """
    ...
    def _load_raw_metric_data(self, **kwargs) -> Dict[str, Any]:
        ...
        for csv_file in csv_files:
            ...
            gt = (
                DFireDSMetricSrc.NEG_LABEL
                if "FP" in video_name
                else DFireDSMetricSrc.POS_LABEL
            )
            df = pd.read_csv(
                csv_file,
                sep=";",
            ...
            df["pred_label"] = (
                ...
                .apply(
                    lambda x: (
                        DFireDSMetricSrc.POS_LABEL
                        if ("fire" in x or "smoke" in x)
                        else DFireDSMetricSrc.NEG_LABEL
                    )
                )
            )
            # pprint(csv_file)
    ...
    def _get_cls_data(self, metric, mode, **kwargs) -> Dict[str, Any]:
        ...
        if metric == "FPS":
            ...
            for per_video_data in per_video_out_list:
                ...
                preds = np.array(preds) == DFireDSMetricSrc.POS_LABEL
                preds = preds.astype(int).tolist()  # convert to int
                gts = per_video_data[0]  # already numpy
                gts = np.array(gts) == DFireDSMetricSrc.POS_LABEL

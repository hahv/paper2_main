...
class BaseMetricDataSource(ABC):
    def __init__(self, dataset_name):
        self.dataset_name = (
            dataset_name  # we must know which dataset we're working with
        )
...
class CSVBinaryMetricDataSource(BaseMetricDataSource):
        ...
        if metric_mode == "per_video":
            if self.dataset_name == "Test":
                ...
                raise NotImplementedError(
                    f"Dataset {self.dataset_name} not supported yet"
                )
        elif metric_mode == "per_frame":
            raise NotImplementedError(
                f"Per-frame mode not implemented for dataset {self.dataset_name}"
            )
    ...
    def get_data(self, **kwargs):
        ...
        # per_video, per_frame
        assert metric_mode in ["per_video", "per_frame"], (
        ...
        if metric_mode == "per_video":
            if self.dataset_name == "Test":
                ...
                gts_tensor = torch.from_numpy(
                    (np.array(gts) == CSVBinaryMetricDataSource.POSITIVE_LABEL).astype(
                        np.int32
                    )
                )
            ...
            raise NotImplementedError(
                f"Metric mode {metric_mode} not implemented for dataset {self.dataset_name}"
            )

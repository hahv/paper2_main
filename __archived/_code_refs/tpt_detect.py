...
def run(
    ...
    if temporal == "tracker":
        ...
        output_name = "tracker_" + output_name + ".csv"
        tracker = ObjectTracker(area_thresh, window_size)
        log = Log()
        threshold = area_thresh
    elif temporal == "persistence":
        print("\nFire detector with temporal persistence\n")
        output_name = "persistence_" + output_name + ".csv"
            ...
            if len(det):
                ...
                if temporal == "tracker":
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        xmin = int(xyxy[0].detach().cpu().numpy())
                        ymin = int(xyxy[1].detach().cpu().numpy())
                        xmax = int(xyxy[2].detach().cpu().numpy())
                        ymax = int(xyxy[3].detach().cpu().numpy())
                        # Confidence score of bounding box
                        conf_score = conf.detach().cpu().numpy()
                        class_id = int(cls.detach().cpu().numpy())
                        # Recording bounding box coordinates
                        log.coord_objs.append([xmin, ymin, xmax, ymax])
                        # Recording class identifier
                        log.classes.append(class_id)
                        # Recording confidence score
                        log.conf_scores.append(conf_score)
                    ...
                    log.classes.clear()
                    log.conf_scores.clear()

                if temporal == "persistence":
                    mask = np.ones((det.shape[0],), dtype=bool)

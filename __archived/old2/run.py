...
def mk_exp_cfg_list(datasets, methods, base_cfg_file, timestamps=None) -> list[Config]:
    ...
    if timestamps is not None:
        assert len(timestamps) == len(datasets) * len(methods), "Number of timestamps must match the number of dataset-method combinations."
...
def main():
    ...
    exp_cfg_dict_ls = mk_exp_cfg_list(datasets, methods, base_cfg_file, timestamps=time_stamps)

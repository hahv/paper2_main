...
from __archived.temporal.__temp_stabilize import ProposedDetector
...
def main():
    ...
    for i, exp_dir in enumerate(exp_dirs):
        ...
        if cfg.dataset.target_ds_name in IGNORE_DS:
            console.print(
                f"[yellow]Skipping dataset {cfg.dataset.target_ds_name} in {exp_dir}"
            )
            continue
        assert os.path.exists(cfg.get_output_dir()), (
            f"Output directory {cfg.get_output_dir()} does not exist."
        )
        # pprint(cfg)
    ...
    def custom_sort_fn(method_ls):
        ...
        assert len(sorted_ls) == len(method_ls), (
            "Custom sort function did not return all methods."
        )
        return sorted_ls

    perftb.plot(
        save_path="./zreport/report_all.svg",
        open_plot=True,
        custom_sort_exp_fn=custom_sort_fn,
    )
